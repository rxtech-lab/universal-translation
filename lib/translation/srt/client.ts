...
import { parseSrt, type SrtCue, serializeSrt } from "./parser";

/** Shape of the format-specific data stored in the DB. */
export interface SrtFormatData {
  cues: SrtCue[];
...
export class SrtClient implements TranslationClient<SrtTranslationEvent> {
  private cues: SrtCue[] = [];
  ...
  async load(payload: UploadPayload): Promise<OperationResult> {
    if (payload.kind !== "single-file") {
      return {
        hasError: true,
        errorMessage: "SRT requires a single file upload",
      };
    }
    ...
    if (this.cues.length === 0) {
      return {
        hasError: true,
        errorMessage: "No valid subtitle cues found in the SRT file",
      };
    }
  ...
  > {
    ...
    const translatedCues: SrtCue[] = this.cues.map((cue) => {
    ...
    const srtContent = serializeSrt(translatedCues);
    const blob = new Blob([srtContent], { type: "text/plain;charset=utf-8" });
    const baseName = this.originalFileName.replace(/\.srt$/i, "");
    const fileName = `${baseName}_${this.targetLanguage || "translated"}.srt`;
  ...
  async open(openProjectId: string): Promise<OperationResult> {
    ...
    const formatData = dbProject.formatData as unknown as SrtFormatData | null;
